...
var AR = ['kya','kyu','kyo', 'sha','shu','sho', 'cha','chu','cho', 'nya','nyu','nyo', 'hya','hyu','hyo', 'mya','myu','myo', 'rya','ryu','ryo', 'gya','gyu','gyo', 'ja','ju','jo', 'bya','byu','byo', 'pya','pyu','pyo'],
...
function fLoad() {      // Whenever possible, I tried to defend against simple errors during loading
  var j;
  if (localStorage.tc_cbHeadings == 'true')        // is table headings shown
    document.getElementById('cbHeadings').checked = true
  else
    document.getElementById('cbHeadings').checked = false;
  fChangeHeadings();

  j = Number(localStorage.tc_gVoice);                    // variant of voice acting, current 0 and 1
  gVoice = (isNaN(j))?0:j;
  ...
  j = Number(localStorage.tc_gLoops);                    // number of voice repeats
  gLoops = (isNaN(j))?1:j;
  document.getElementById('repeatNumber').value = gLoops;

  j = Number(localStorage.tc_gPause);                    // pause between voice acting
  gPause = (isNaN(j))?800:j;
  document.getElementById('repeatPause').value = gPause/1000;

  if (localStorage.tc_gVolume !== undefined && localStorage.tc_gVolume !== 'undefined')      // volume of audio, array
    gVolume = localStorage.tc_gVolume.split(',')
  else
    gVolume = [0.8, 0.5];
  ...
  if (localStorage.tc_gKana == 'K')                                              // kana type
    gKana = 'K'
  else
    gKana = 'H';
  ...
  if (localStorage.tc_gCorner == 'K' || localStorage.tc_gCorner == 'H')        // kana type
    gCorner = localStorage.tc_gCorner
  else
    gCorner = 'R';
...
function fUnload() {
  localStorage.setItem('tc_gVoice', gVoice);
  localStorage.setItem('tc_gLoops', gLoops);
  localStorage.setItem('tc_gPause', gPause);
  localStorage.setItem('tc_gVolume', gVolume);
  localStorage.setItem('tc_gKana', gKana);
  localStorage.setItem('tc_gCorner', gCorner);
  localStorage.setItem('tc_cbHeadings', document.getElementById('cbHeadings').checked);
}
    ...
    if (firstMove) {
      ...
      switch (k) {
        ...
        case 40:
          iAR+=3;
          if (iAR==35) iAR = 0
          else if (iAR>=33) iAR-=32;
          break;
        case 38:
          iAR-=3;
          if (iAR==-3) iAR = 32
          else if (iAR<0) iAR+=32;
          break;
        case 27:
          document.getElementById(AR[iAR]).lastChild.style.borderColor = 'transparent';
          return;
